!function(modules){function __webpack_require__(moduleId){if(installedModules[moduleId])return installedModules[moduleId].exports;var module=installedModules[moduleId]={i:moduleId,l:!1,exports:{}};return modules[moduleId].call(module.exports,module,module.exports,__webpack_require__),module.l=!0,module.exports}var installedModules={};return __webpack_require__.m=modules,__webpack_require__.c=installedModules,__webpack_require__.i=function(value){return value},__webpack_require__.d=function(exports,name,getter){__webpack_require__.o(exports,name)||Object.defineProperty(exports,name,{configurable:!1,enumerable:!0,get:getter})},__webpack_require__.n=function(module){var getter=module&&module.__esModule?function(){return module.default}:function(){return module};return __webpack_require__.d(getter,"a",getter),getter},__webpack_require__.o=function(object,property){return Object.prototype.hasOwnProperty.call(object,property)},__webpack_require__.p="",__webpack_require__(__webpack_require__.s=11)}([function(module,exports,__webpack_require__){"use strict";var $=__webpack_require__(1),render=function(){function activePlayer(activePlayer){var $activeScoreContainer=$el.find(activePlayer.scoreContainer),$scoreContainer=$el.find(".scoreContainer");$scoreContainer.removeClass("activePlayer"),$activeScoreContainer.addClass("activePlayer")}function balance(total){$balance.text(total)}function bet(bet,activePlayer){var $icon=$("<img class='bet-icon' src='./icons/CHIP"+bet+".png'/>"),$userBetContainer=$el.find(activePlayer.betContainer);activePlayer.statusDouble||$userBetContainer.empty(),$icon.appendTo($userBetContainer)}function createDealer(){var dealerHTML="\n    <div><div id='cardsDealer'></div></div>\n    <div><h2 class='scoreContainer' id='scoreDealer'></h2></div>";$dealerContainer.append(dealerHTML)}function createUser(index,split,oldPlayer){var symbol=0;symbol=index%2!==0?"./icons/User2.png":"./icons/User.png";var userHTML="\n    <div class='users' id='user"+index+"'>\n      <h2 class='scoreDiv scoreContainer image-container' id='scoreUser"+index+"'></h2>\n      <div class='cardsDiv' id='cardsUser"+index+"'></div>\n      <div class='image-container'>\n        <img class='user-icon image-context' src="+symbol+"></img>\n        <div class='betDiv image-context' id='betUser"+index+"'></div>\n      </div>\n    </div>";if(split){var $splitUserContainer=$el.find(oldPlayer.userContainer);$splitUserContainer.before(userHTML)}else $usersContainer.append(userHTML)}function doubleButton(){$doubleElement.toggle()}function eventLog(text){$eventLog.html()!==text&&$eventLog.slideUp(400,function(){$eventLog.empty(),$eventLog.text(text).hide().slideDown(400)})}function hideCardDealer(status,activePlayer){var $cardsDealer=$el.find("#cardsDealer");status?$cardsDealer.children()[1].src="./icons/hidden.png":status||($cardsDealer.children()[1].src=activePlayer.cards[1].src)}function pushCard(card,activePlayer){var $cardContainer=$el.find(activePlayer.cardContainer),$image=$('<img class="card-icon" src='+card.src+" />");$image.appendTo($cardContainer).hide().slideDown("slow"),activePlayer.cards.length>1&&$image.css({"margin-left":"-44px"}),"Dealer"===activePlayer.id&&2===activePlayer.cards.length&&hideCardDealer("On")}function removeSplitCard(player){var $cardContainer=$el.find(player.cardContainer);$cardContainer.children()[1].remove()}function removeUserHTML(obsoleteUser){var $userContainer=$el.find(obsoleteUser.userContainer);$userContainer.remove()}function resetPlayer(player){var $betContainer=$el.find(player.betContainer),$cardContainer=$el.find(player.cardContainer),$scoreContainer=$el.find(player.scoreContainer);$betContainer.empty(),$cardContainer.empty(),$scoreContainer.empty()}function result(user){var $resultContainer=$('<p class="result-text image-context">'+user.result[0]+"</p>"),$scoreContainer=$el.find(user.scoreContainer);$resultContainer.appendTo($scoreContainer)}function rules(){var $rulesHTML=$("\n    <button onclick='this.remove()' id='rules'>\n      BLACKJACK RULES<br>\n      <br>\n      - Press NEW GAME to start<br>\n      <br>\n      - The object of the game is to reach 21 points<br>\n        or achieve a score higher than the dealer<br>\n        without exceeding 21<br>\n      <br>\n      - First, place a bet and press DEAL<br>\n      <br>\n      - Then HIT to add a card, or STAND if you are<br>\n        happy with your cards<br>\n      <br>\n      - If your card total goes over 21 you are BUST<br>\n        and the dealer wins<br>\n      <br>\n      - You can SPLIT your cards if dealt two cards<br>\n        of the same value<br>\n      <br>\n      - Double your bet if you are dealt a total score<br>\n        of 9, 10 or 11. You will receive only one<br>\n        further card<br>\n    </button>");$rulesHTML.appendTo($el)}function score(activePlayer){var $scoreContainer=$el.find(activePlayer.scoreContainer);$scoreContainer.text(activePlayer.netScore)}function splitButton(){$splitElement.toggle()}function volumeMute(status){var $soundIcon=$el.find("#sound-src");status?$soundIcon.attr("src","./icons/SOUND.png"):$soundIcon.attr("src","./icons/SOUNDMUTE.png")}var $el=$(".blackjackModule"),$balance=$el.find("#balance"),$dealerContainer=$el.find("#dealerContainer"),$doubleElement=$el.find("#doubleElement"),$eventLog=$el.find("#eventLog"),$splitElement=$el.find("#splitElement"),$usersContainer=$el.find("#usersContainer");return{activePlayer:activePlayer,balance:balance,bet:bet,createDealer:createDealer,createUser:createUser,doubleButton:doubleButton,eventLog:eventLog,hideCardDealer:hideCardDealer,pushCard:pushCard,removeSplitCard:removeSplitCard,removeUserHTML:removeUserHTML,resetPlayer:resetPlayer,result:result,rules:rules,score:score,splitButton:splitButton,volumeMute:volumeMute}}();module.exports=render},function(module,exports,__webpack_require__){var __WEBPACK_AMD_DEFINE_ARRAY__,__WEBPACK_AMD_DEFINE_RESULT__;/*!
...
options.crossDomain||headers["X-Requested-With"]||(headers["X-Requested-With"]="XMLHttpRequest");for(i in headers)xhr.setRequestHeader(i,headers[i]);callback=function(type){return function(){callback&&(callback=errorCallback=xhr.onload=xhr.onerror=xhr.onabort=xhr.onreadystatechange=null,"abort"===type?xhr.abort():"error"===type?"number"!=typeof xhr.status?complete(0,"error"):complete(xhr.status,xhr.statusText):complete(xhrSuccessStatus[xhr.status]||xhr.status,xhr.statusText,"text"!==(xhr.responseType||"text")||"string"!=typeof xhr.responseText?{binary:xhr.response}:{text:xhr.responseText},xhr.getAllResponseHeaders()))}},xhr.onload=callback(),errorCallback=xhr.onerror=callback("error"),void 0!==xhr.onabort?xhr.onabort=errorCallback:xhr.onreadystatechange=function(){4===xhr.readyState&&window.setTimeout(function(){callback&&errorCallback()})},callback=callback("abort");try{xhr.send(options.hasContent&&options.data||null)}catch(e){if(callback)throw e}},abort:function(){callback&&callback()}}}),jQuery.ajaxPrefilter(function(s){s.crossDomain&&(s.contents.script=!1)}),jQuery.ajaxSetup({accepts:{script:"text/javascript, application/javascript, application/ecmascript, application/x-ecmascript"},contents:{script:/\b(?:java|ecma)script\b/},converters:{"text script":function(text){return jQuery.globalEval(text),text}}}),jQuery.ajaxPrefilter("script",function(s){void 0===s.cache&&(s.cache=!1),s.crossDomain&&(s.type="GET")}),jQuery.ajaxTransport("script",function(s){if(s.crossDomain){var script,callback;return{send:function(_,complete){script=jQuery("<script>").prop({charset:s.scriptCharset,src:s.url}).on("load error",callback=function(evt){script.remove(),callback=null,evt&&complete("error"===evt.type?404:200,evt.type)}),document.head.appendChild(script[0])},abort:function(){callback&&callback()}}}});var oldCallbacks=[],rjsonp=/(=)\?(?=&|$)|\?\?/;jQuery.ajaxSetup({jsonp:"callback",jsonpCallback:function(){var callback=oldCallbacks.pop()||jQuery.expando+"_"+nonce++;return this[callback]=!0,callback}}),jQuery.ajaxPrefilter("json jsonp",function(s,originalSettings,jqXHR){var callbackName,overwritten,responseContainer,jsonProp=s.jsonp!==!1&&(rjsonp.test(s.url)?"url":"string"==typeof s.data&&0===(s.contentType||"").indexOf("application/x-www-form-urlencoded")&&rjsonp.test(s.data)&&"data");if(jsonProp||"jsonp"===s.dataTypes[0])return callbackName=s.jsonpCallback=jQuery.isFunction(s.jsonpCallback)?s.jsonpCallback():s.jsonpCallback,jsonProp?s[jsonProp]=s[jsonProp].replace(rjsonp,"$1"+callbackName):s.jsonp!==!1&&(s.url+=(rquery.test(s.url)?"&":"?")+s.jsonp+"="+callbackName),s.converters["script json"]=function(){return responseContainer||jQuery.error(callbackName+" was not called"),responseContainer[0]},s.dataTypes[0]="json",overwritten=window[callbackName],window[callbackName]=function(){responseContainer=arguments},jqXHR.always(function(){void 0===overwritten?jQuery(window).removeProp(callbackName):window[callbackName]=overwritten,s[callbackName]&&(s.jsonpCallback=originalSettings.jsonpCallback,oldCallbacks.push(callbackName)),responseContainer&&jQuery.isFunction(overwritten)&&overwritten(responseContainer[0]),responseContainer=overwritten=void 0}),"script"}),support.createHTMLDocument=function(){var body=document.implementation.createHTMLDocument("").body;return body.innerHTML="<form></form><form></form>",2===body.childNodes.length}(),jQuery.parseHTML=function(data,context,keepScripts){if("string"!=typeof data)return[];"boolean"==typeof context&&(keepScripts=context,context=!1);var base,parsed,scripts;return context||(support.createHTMLDocument?(context=document.implementation.createHTMLDocument(""),base=context.createElement("base"),base.href=document.location.href,context.head.appendChild(base)):context=document),parsed=rsingleTag.exec(data),scripts=!keepScripts&&[],parsed?[context.createElement(parsed[1])]:(parsed=buildFragment([data],context,scripts),scripts&&scripts.length&&jQuery(scripts).remove(),jQuery.merge([],parsed.childNodes))},jQuery.fn.load=function(url,params,callback){var selector,type,response,self=this,off=url.indexOf(" ");return off>-1&&(selector=stripAndCollapse(url.slice(off)),url=url.slice(0,off)),jQuery.isFunction(params)?(callback=params,params=void 0):params&&"object"==typeof params&&(type="POST"),self.length>0&&jQuery.ajax({url:url,type:type||"GET",dataType:"html",data:params}).done(function(responseText){response=arguments,self.html(selector?jQuery("<div>").append(jQuery.parseHTML(responseText)).find(selector):responseText)}).always(callback&&function(jqXHR,status){self.each(function(){callback.apply(this,response||[jqXHR.responseText,status,jqXHR])})}),this},jQuery.each(["ajaxStart","ajaxStop","ajaxComplete","ajaxError","ajaxSuccess","ajaxSend"],function(i,type){jQuery.fn[type]=function(fn){return this.on(type,fn)}}),jQuery.expr.pseudos.animated=function(elem){return jQuery.grep(jQuery.timers,function(fn){return elem===fn.elem}).length},jQuery.offset={setOffset:function(elem,options,i){var curPosition,curLeft,curCSSTop,curTop,curOffset,curCSSLeft,calculatePosition,position=jQuery.css(elem,"position"),curElem=jQuery(elem),props={};"static"===position&&(elem.style.position="relative"),curOffset=curElem.offset(),curCSSTop=jQuery.css(elem,"top"),curCSSLeft=jQuery.css(elem,"left"),calculatePosition=("absolute"===position||"fixed"===position)&&(curCSSTop+curCSSLeft).indexOf("auto")>-1,calculatePosition?(curPosition=curElem.position(),curTop=curPosition.top,curLeft=curPosition.left):(curTop=parseFloat(curCSSTop)||0,curLeft=parseFloat(curCSSLeft)||0),jQuery.isFunction(options)&&(options=options.call(elem,i,jQuery.extend({},curOffset))),null!=options.top&&(props.top=options.top-curOffset.top+curTop),null!=options.left&&(props.left=options.left-curOffset.left+curLeft),"using"in options?options.using.call(elem,props):curElem.css(props)}},jQuery.fn.extend({offset:function(options){if(arguments.length)return void 0===options?this:this.each(function(i){jQuery.offset.setOffset(this,options,i)});var docElem,win,rect,doc,elem=this[0];if(elem)return elem.getClientRects().length?(rect=elem.getBoundingClientRect(),rect.width||rect.height?(doc=elem.ownerDocument,win=getWindow(doc),docElem=doc.documentElement,{top:rect.top+win.pageYOffset-docElem.clientTop,left:rect.left+win.pageXOffset-docElem.clientLeft}):rect):{top:0,left:0}},position:function(){if(this[0]){var offsetParent,offset,elem=this[0],parentOffset={top:0,left:0};return"fixed"===jQuery.css(elem,"position")?offset=elem.getBoundingClientRect():(offsetParent=this.offsetParent(),offset=this.offset(),jQuery.nodeName(offsetParent[0],"html")||(parentOffset=offsetParent.offset()),parentOffset={top:parentOffset.top+jQuery.css(offsetParent[0],"borderTopWidth",!0),left:parentOffset.left+jQuery.css(offsetParent[0],"borderLeftWidth",!0)}),{top:offset.top-parentOffset.top-jQuery.css(elem,"marginTop",!0),left:offset.left-parentOffset.left-jQuery.css(elem,"marginLeft",!0)}}},offsetParent:function(){return this.map(function(){for(var offsetParent=this.offsetParent;offsetParent&&"static"===jQuery.css(offsetParent,"position");)offsetParent=offsetParent.offsetParent;return offsetParent||documentElement})}}),jQuery.each({scrollLeft:"pageXOffset",scrollTop:"pageYOffset"},function(method,prop){var top="pageYOffset"===prop;jQuery.fn[method]=function(val){return access(this,function(elem,method,val){var win=getWindow(elem);return void 0===val?win?win[prop]:elem[method]:void(win?win.scrollTo(top?win.pageXOffset:val,top?val:win.pageYOffset):elem[method]=val)},method,val,arguments.length)}}),jQuery.each(["top","left"],function(i,prop){jQuery.cssHooks[prop]=addGetHookIf(support.pixelPosition,function(elem,computed){if(computed)return computed=curCSS(elem,prop),rnumnonpx.test(computed)?jQuery(elem).position()[prop]+"px":computed})}),jQuery.each({Height:"height",Width:"width"},function(name,type){jQuery.each({padding:"inner"+name,content:type,"":"outer"+name},function(defaultExtra,funcName){jQuery.fn[funcName]=function(margin,value){var chainable=arguments.length&&(defaultExtra||"boolean"!=typeof margin),extra=defaultExtra||(margin===!0||value===!0?"margin":"border");return access(this,function(elem,type,value){var doc;return jQuery.isWindow(elem)?0===funcName.indexOf("outer")?elem["inner"+name]:elem.document.documentElement["client"+name]:9===elem.nodeType?(doc=elem.documentElement,Math.max(elem.body["scroll"+name],doc["scroll"+name],elem.body["offset"+name],doc["offset"+name],doc["client"+name])):void 0===value?jQuery.css(elem,type,extra):jQuery.style(elem,type,value,extra)},type,chainable?margin:void 0,chainable)}})}),jQuery.fn.extend({bind:function(types,data,fn){return this.on(types,null,data,fn)},unbind:function(types,fn){return this.off(types,null,fn)},delegate:function(selector,types,data,fn){return this.on(types,selector,data,fn)},undelegate:function(selector,types,fn){return 1===arguments.length?this.off(selector,"**"):this.off(types,selector||"**",fn)}}),jQuery.parseJSON=JSON.parse,__WEBPACK_AMD_DEFINE_ARRAY__=[],__WEBPACK_AMD_DEFINE_RESULT__=function(){return jQuery}.apply(exports,__WEBPACK_AMD_DEFINE_ARRAY__),!(void 0!==__WEBPACK_AMD_DEFINE_RESULT__&&(module.exports=__WEBPACK_AMD_DEFINE_RESULT__));var _jQuery=window.jQuery,_$=window.$;return jQuery.noConflict=function(deep){return window.$===jQuery&&(window.$=_$),deep&&window.jQuery===jQuery&&(window.jQuery=_jQuery),jQuery},noGlobal||(window.jQuery=window.$=jQuery),jQuery})},function(module,exports,__webpack_require__){"use strict";function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor))throw new TypeError("Cannot call a class as a function")}var _createClass=function(){function defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||!1,descriptor.configurable=!0,"value"in descriptor&&(descriptor.writable=!0),Object.defineProperty(target,descriptor.key,descriptor)}}return function(Constructor,protoProps,staticProps){return protoProps&&defineProperties(Constructor.prototype,protoProps),staticProps&&defineProperties(Constructor,staticProps),Constructor}}(),render=__webpack_require__(0),Player=function(){function Player(){_classCallCheck(this,Player),this.aces=0,this.bet=0,this.cards=[],this.grossScore=0,this.inplay=!1,this.netScore=0,this.status="",this.statusDouble=!1,this.result=[]}return _createClass(Player,[{key:"addCard",value:function(card,activePlayer){this.cards.push(card),this.grossScore+=card.value,this.netScore+=card.value,this.countAces(card),this.adjustAces(),render.pushCard(card,activePlayer),render.score(activePlayer),this.netScore>21&&(this.status="Bust"),21===this.netScore&&(this.status="Stand"),2===this.cards.length&&21===this.netScore&&(this.status="Blackjack!")}},{key:"adjustAces",value:function(){this.netScore<22||this.grossScore-10*this.aces>21||(this.netScore-=10)}},{key:"countAces",value:function(card){"ace"===card.number&&(this.aces+=1)}},{key:"reset",value:function(player){this.aces=0,this.bet=0,this.cards=[],this.grossScore=0,this.inplay=!1,this.netScore=0,this.status="",this.statusDouble=!1,this.result="",render.resetPlayer(player)}}]),Player}();module.exports=Player},function(module,exports,__webpack_require__){"use strict";var $=__webpack_require__(1),render=__webpack_require__(0),buttons=function(){function toggleAllOff(){this.toggleStandAndHit(),$double.is(":disabled")||this.toggleDouble(),$split.is(":disabled")||this.toggleSplit()}function toggleBet(balance){$twoHundred.prop("disabled",!0),$hundred.prop("disabled",!0),$fifty.prop("disabled",!0),$twentyFive.prop("disabled",!0),balance>=200&&$twoHundred.prop("disabled",!1),balance>=100&&$hundred.prop("disabled",!1),balance>=50&&$fifty.prop("disabled",!1),balance>=25&&$twentyFive.prop("disabled",!1)}function toggleDouble(){$double.prop("disabled",function(i,v){return!v}),render.doubleButton()}function toggleGame(status){status?$game.prop("disabled",!1):$game.prop("disabled",function(i,v){return!v})}function toggleSplit(){$split.prop("disabled",function(i,v){return!v}),render.splitButton()}function toggleStandAndHit(){$stand.prop("disabled",function(i,v){return!v}),$takeCard.prop("disabled",function(i,v){return!v})}var $el=$(".blackjackModule"),$double=$el.find("#double"),$fifty=$el.find("#fifty"),$game=$el.find("#game"),$hundred=$el.find("#hundred"),$split=$el.find("#split"),$stand=$el.find("#stand"),$takeCard=$el.find("#takeCard"),$twentyFive=$el.find("#twentyFive"),$twoHundred=$el.find("#twoHundred");return{toggleAllOff:toggleAllOff,toggleBet:toggleBet,toggleDouble:toggleDouble,toggleGame:toggleGame,toggleSplit:toggleSplit,toggleStandAndHit:toggleStandAndHit}}();module.exports=buttons},function(module,exports,__webpack_require__){"use strict";var $=__webpack_require__(1),Balance=__webpack_require__(5),buttons=__webpack_require__(3),Dealer=__webpack_require__(7),Deck=__webpack_require__(8),render=__webpack_require__(0),sounds=__webpack_require__(10),User=__webpack_require__(9),game=function(){function allUsersBustOrBlackjackorNotInplay(){for(var index=0,numberOfUsers=players.length-1;index<numberOfUsers;index++)if("Bust"!==players[index].status&&"Blackjack!"!==players[index].status&&players[index].inplay!==!1)return!1;return!0}function betPlaced(bet){sounds.chips(),activePlayer.setBet(bet,activePlayer),gameButtonStatusNewGame=!1,buttons.toggleGame(!0),render.eventLog("Deal")}function createPlayers(){var index=0;if(users>maxUsers||users<1)return buttons.toggleGame(),void alert("You're a funny guy");for(;index<users;index++)players.push(new User(index));players.push(new Dealer)}function deal(){var indexUser=players.indexOf(activePlayer)+1;sounds.buttonPress(),balance.reduce(activePlayer.bet),activePlayer.inplay=!0,indexUser<users&&!balance.insufficientFunds()?(activePlayer=selectNextPlayer(activePlayer),buttons.toggleGame(),buttons.toggleBet(balance.total)):(gameButtonStatusNewGame=!0,buttons.toggleGame(),buttons.toggleBet(),activePlayer=players[0],startDealing())}function dealersTurn(){function _dealerTakeCards(callback){setTimeout(function(){activePlayer.netScore<17?(sounds.drawCard(),activePlayer.addCard(deck.grabCard(),activePlayer),_dealerTakeCards(callback)):callback&&callback()},800)}activePlayer.hideCardDealer(!1,activePlayer),allUsersBustOrBlackjackorNotInplay()?determineResults():_dealerTakeCards(determineResults)}function determineResults(){function _userResult(){setTimeout(function(){user=players[index],"Blackjack!"===user.status&&"Blackjack!"===dealer.status?(sounds.draw(),user.result=["Draw",1]):"Blackjack!"===user.status&&"Blackjack!"!==dealer.status?(sounds.win(),user.result=["Win!",2.5]):"Blackjack!"!==user.status&&"Blackjack!"===dealer.status?(sounds.lose(),user.result=["Lose",0]):"Bust"===user.status?(sounds.lose(),user.result=["Lose",0]):dealer.netScore>21||user.netScore>dealer.netScore?(sounds.win(),user.result=["Win!",2]):user.netScore===dealer.netScore?(sounds.draw(),user.result=["Draw",1]):dealer.netScore>user.netScore&&(sounds.lose(),user.result=["Lose",0]),render.result(user),balance.procesResult(user),index++,index<numberOfUsers&&players[index].inplay===!0?_userResult():endGame()},1e3)}var dealer=activePlayer,index=0,numberOfUsers=players.length-1,user={};_userResult()}function endGame(){balance.insufficientFunds()?(sounds.gameOver(),render.eventLog("Game Over"),setTimeout(function(){render.eventLog("Try Again"),balance.set(1e3),buttons.toggleGame()},2e3)):(render.eventLog("Next Game"),buttons.toggleGame())}function gameButtonSwitch(){gameButtonStatusNewGame?newGame():gameButtonStatusNewGame||deal()}function gameMechanics(){function _nextPlayerPlays(){activePlayer=selectNextPlayer(activePlayer),activePlayer.inplay===!1&&(numberOfPlayers=players.length-1,activePlayer=players[numberOfPlayers]),render.activePlayer(activePlayer),"Dealer"===activePlayer.id?dealersTurn():(render.eventLog("Play"),gameMechanics())}var numberOfPlayers=0;activePlayer.status?(render.eventLog(activePlayer.status),"Blackjack!"===activePlayer.status?sounds.blackjack():"Bust"===activePlayer.status&&sounds.bust(),setTimeout(function(){_nextPlayerPlays()},1500)):activePlayer.determineOptions(users,maxUsers,balance.total)}function info(){sounds.buttonPress(),render.rules()}function newGame(){var index=0,obsoleteUser={},player={};sounds.buttonPress();var _iteratorNormalCompletion=!0,_didIteratorError=!1,_iteratorError=void 0;try{for(var _step,_iterator=players[Symbol.iterator]();!(_iteratorNormalCompletion=(_step=_iterator.next()).done);_iteratorNormalCompletion=!0)player=_step.value,player.reset(player)}catch(err){_didIteratorError=!0,_iteratorError=err}finally{try{!_iteratorNormalCompletion&&_iterator.return&&_iterator.return()}finally{if(_didIteratorError)throw _iteratorError}}var _iteratorNormalCompletion2=!0,_didIteratorError2=!1,_iteratorError2=void 0;try{for(var _step2,_iterator2=players[Symbol.iterator]();!(_iteratorNormalCompletion2=(_step2=_iterator2.next()).done);_iteratorNormalCompletion2=!0)player=_step2.value,"Split"===player.statusSplit&&(index=players.indexOf(player),obsoleteUser=players[index],obsoleteUser.removeHTML(obsoleteUser),players.splice(index,1),users--)}catch(err){_didIteratorError2=!0,_iteratorError2=err}finally{try{!_iteratorNormalCompletion2&&_iterator2.return&&_iterator2.return()}finally{if(_didIteratorError2)throw _iteratorError2}}deck.shuffle(),buttons.toggleGame(),activePlayer=players[0],render.activePlayer(activePlayer),render.eventLog("Place Bet"),buttons.toggleBet(balance.total)}function selectNextPlayer(activePlayer){var index=players.indexOf(activePlayer);return index+=1,index%players.length===0&&(index=0),players[index]}function selectNextPlayerInPlay(activePlayer){var index=players.indexOf(activePlayer),numberOfPlayers=players.length,player={};return index+=1,index%numberOfPlayers===0&&(index=0),player=players[index],player.inplay===!0?player:players[numberOfPlayers-1]}function start(setOpeningBalance,setPacks,setUsersmin,setUsersmax){balance=new Balance(setOpeningBalance),deck=new Deck(setPacks),deck.create(),users=setUsersmin,maxUsers=setUsersmax,createPlayers(),buttons.toggleGame()}function startDealing(){function _dealCard(){setTimeout(function(){sounds.drawCard(),activePlayer.addCard(deck.grabCard(),activePlayer),"Dealer"===activePlayer.id&&2===activePlayer.cards.length&&activePlayer.hideCardDealer(!0,activePlayer),activePlayer=selectNextPlayerInPlay(activePlayer),activePlayer!==players[0]||2!==activePlayer.cards.length?_dealCard():(activePlayer=players[0],render.activePlayer(activePlayer),render.eventLog("Select Action"),gameMechanics())},400)}_dealCard()}function userDouble(){sounds.buttonPress(),balance.reduce(activePlayer.bet),activePlayer.doubleBet(activePlayer),activePlayer.processDoubleScore(activePlayer),activePlayer.addCard(deck.grabCard(),activePlayer),activePlayer.status="Stand",buttons.toggleAllOff(),gameMechanics()}function userSplit(){var index=0,newPlayer={},oldPlayer={},transferCard={};sounds.buttonPress(),index=players.indexOf(activePlayer),oldPlayer=players[index],transferCard=oldPlayer.removeSplitCard(oldPlayer),players.splice(index,0,new User(users,"Split",oldPlayer)),newPlayer=players[index],users++,newPlayer.inplay=!0,newPlayer.setBet(oldPlayer.bet,newPlayer),balance.reduce(newPlayer.bet),newPlayer.addCard(transferCard,newPlayer),oldPlayer.addCard(deck.grabCard(),oldPlayer),newPlayer.addCard(deck.grabCard(),newPlayer),activePlayer=newPlayer,render.activePlayer(activePlayer),buttons.toggleAllOff(),gameMechanics()}function userStand(){sounds.buttonPress(),activePlayer.status="Stand",buttons.toggleAllOff(),gameMechanics()}function userTakeCard(){sounds.buttonPress(),activePlayer.addCard(deck.grabCard(),activePlayer),buttons.toggleAllOff(),gameMechanics()}function volume(){sounds.toggleVolume()}var activePlayer={},balance={},deck={},gameButtonStatusNewGame=!0,maxUsers=0,players=[],users=0,$el=$(".blackjackModule"),$double=$el.find("#double"),$fifty=$el.find("#fifty"),$game=$el.find("#game"),$hundred=$el.find("#hundred"),$info=$el.find("#info"),$split=$el.find("#split"),$stand=$el.find("#stand"),$takeCard=$el.find("#takeCard"),$twentyFive=$el.find("#twentyFive"),$twoHundred=$el.find("#twoHundred"),$volume=$el.find("#volume");return $double.click(userDouble),$fifty.click(function(){betPlaced(50)}),$game.click(gameButtonSwitch),$hundred.click(function(){betPlaced(100)}),$info.click(info),$split.click(userSplit),$stand.click(userStand),$takeCard.click(userTakeCard),$twentyFive.click(function(){betPlaced(25)}),$twoHundred.click(function(){betPlaced(200)}),$volume.click(volume),{start:start}}();module.exports=game},function(module,exports,__webpack_require__){"use strict";function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor))throw new TypeError("Cannot call a class as a function")}var _createClass=function(){function defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||!1,descriptor.configurable=!0,"value"in descriptor&&(descriptor.writable=!0),Object.defineProperty(target,descriptor.key,descriptor)}}return function(Constructor,protoProps,staticProps){return protoProps&&defineProperties(Constructor.prototype,protoProps),staticProps&&defineProperties(Constructor,staticProps),Constructor}}(),render=__webpack_require__(0),Balance=function(){function Balance(start){_classCallCheck(this,Balance),this.total=start,render.balance(this.total)}return _createClass(Balance,[{key:"insufficientFunds",value:function(){if(this.total<25)return!0}},{key:"procesResult",value:function(user){this.total=this.total+user.bet*user.result[1],render.balance(this.total)}},{key:"reduce",value:function(bet){this.total-=bet,render.balance(this.total)}},{key:"set",value:function(amount){this.total=amount,render.balance(this.total)}}]),Balance}();module.exports=Balance},function(module,exports,__webpack_require__){"use strict";function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor))throw new TypeError("Cannot call a class as a function")}var Card=function Card(specie,number,value){_classCallCheck(this,Card),this.specie=specie,this.number=number,this.value=value,this.src="./icons/"+specie+"-"+number+".png"};module.exports=Card},function(module,exports,__webpack_require__){"use strict";function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor))throw new TypeError("Cannot call a class as a function")}function _possibleConstructorReturn(self,call){if(!self)throw new ReferenceError("this hasn't been initialised - super() hasn't been called");return!call||"object"!=typeof call&&"function"!=typeof call?self:call}function _inherits(subClass,superClass){if("function"!=typeof superClass&&null!==superClass)throw new TypeError("Super expression must either be null or a function, not "+typeof superClass);subClass.prototype=Object.create(superClass&&superClass.prototype,{constructor:{value:subClass,enumerable:!1,writable:!0,configurable:!0}}),superClass&&(Object.setPrototypeOf?Object.setPrototypeOf(subClass,superClass):subClass.__proto__=superClass)}var _createClass=function(){function defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||!1,descriptor.configurable=!0,"value"in descriptor&&(descriptor.writable=!0),Object.defineProperty(target,descriptor.key,descriptor)}}return function(Constructor,protoProps,staticProps){return protoProps&&defineProperties(Constructor.prototype,protoProps),staticProps&&defineProperties(Constructor,staticProps),Constructor}}(),Player=__webpack_require__(2),render=__webpack_require__(0),Dealer=function(_Player){function Dealer(){_classCallCheck(this,Dealer);var _this=_possibleConstructorReturn(this,(Dealer.__proto__||Object.getPrototypeOf(Dealer)).call(this));return render.createDealer(),_this.id="Dealer",_this.scoreContainer="#scoreDealer",_this.cardContainer="#cardsDealer",_this}return _inherits(Dealer,_Player),_createClass(Dealer,[{key:"hideCardDealer",value:function(status,activePlayer){status?(this.netScore=this.netScore-this.cards[1].value,"ace"===this.cards[0].number&&"ace"===this.cards[1].number&&(this.netScore=this.netScore+10),render.score(activePlayer)):status||(this.netScore=this.netScore+this.cards[1].value,"ace"===this.cards[0].number&&"ace"===this.cards[1].number&&(this.netScore=this.netScore-10),render.score(activePlayer)),render.hideCardDealer(status,activePlayer)}}]),Dealer}(Player);module.exports=Dealer},function(module,exports,__webpack_require__){"use strict";function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor))throw new TypeError("Cannot call a class as a function")}var _createClass=function(){function defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||!1,descriptor.configurable=!0,"value"in descriptor&&(descriptor.writable=!0),Object.defineProperty(target,descriptor.key,descriptor)}}return function(Constructor,protoProps,staticProps){return protoProps&&defineProperties(Constructor.prototype,protoProps),staticProps&&defineProperties(Constructor,staticProps),Constructor}}(),Card=__webpack_require__(6),Deck=function(){function Deck(setPacks){_classCallCheck(this,Deck),this.currentDeck=[],this.packs=setPacks}return _createClass(Deck,[{key:"create",value:function(){for(var index=0,number="",numbers=["two","three","four","five","six","seven","eight","nine","ten","jack","queen","king","ace"],packsCount=0,packsInGame=this.packs,specie="",species=["hearts","spades","diamonds","clubs"],value=0,values=[2,3,4,5,6,7,8,9,10,10,10,10,11];packsCount<packsInGame;packsCount++){var _iteratorNormalCompletion=!0,_didIteratorError=!1,_iteratorError=void 0;try{for(var _step,_iterator=species[Symbol.iterator]();!(_iteratorNormalCompletion=(_step=_iterator.next()).done);_iteratorNormalCompletion=!0){specie=_step.value;var _iteratorNormalCompletion2=!0,_didIteratorError2=!1,_iteratorError2=void 0;try{for(var _step2,_iterator2=numbers[Symbol.iterator]();!(_iteratorNormalCompletion2=(_step2=_iterator2.next()).done);_iteratorNormalCompletion2=!0)number=_step2.value,index=numbers.indexOf(number),value=values[index],this.currentDeck.push(new Card(specie,number,value))}catch(err){_didIteratorError2=!0,_iteratorError2=err}finally{try{!_iteratorNormalCompletion2&&_iterator2.return&&_iterator2.return()}finally{if(_didIteratorError2)throw _iteratorError2}}}}catch(err){_didIteratorError=!0,_iteratorError=err}finally{try{!_iteratorNormalCompletion&&_iterator.return&&_iterator.return()}finally{if(_didIteratorError)throw _iteratorError}}}}},{key:"grabCard",value:function(){var card={},randomNumber=0;return randomNumber=Math.floor(Math.random()*this.currentDeck.length),card=this.currentDeck[randomNumber],this.currentDeck.splice(randomNumber,1),card}},{key:"shuffle",value:function(){this.currentDeck.length<=52*this.packs/2&&(this.currentDeck=[],this.create())}}]),Deck}();module.exports=Deck},function(module,exports,__webpack_require__){"use strict";function _classCallCheck(instance,Constructor){if(!(instance instanceof Constructor))throw new TypeError("Cannot call a class as a function")}function _possibleConstructorReturn(self,call){if(!self)throw new ReferenceError("this hasn't been initialised - super() hasn't been called");return!call||"object"!=typeof call&&"function"!=typeof call?self:call}function _inherits(subClass,superClass){if("function"!=typeof superClass&&null!==superClass)throw new TypeError("Super expression must either be null or a function, not "+typeof superClass);subClass.prototype=Object.create(superClass&&superClass.prototype,{constructor:{value:subClass,enumerable:!1,writable:!0,configurable:!0}}),superClass&&(Object.setPrototypeOf?Object.setPrototypeOf(subClass,superClass):subClass.__proto__=superClass)}var _createClass=function(){function defineProperties(target,props){for(var i=0;i<props.length;i++){var descriptor=props[i];descriptor.enumerable=descriptor.enumerable||!1,descriptor.configurable=!0,"value"in descriptor&&(descriptor.writable=!0),Object.defineProperty(target,descriptor.key,descriptor)}}return function(Constructor,protoProps,staticProps){return protoProps&&defineProperties(Constructor.prototype,protoProps),staticProps&&defineProperties(Constructor,staticProps),Constructor}}(),buttons=__webpack_require__(3),Player=__webpack_require__(2),render=__webpack_require__(0),User=function(_Player){function User(index,split,oldPlayer){_classCallCheck(this,User);var _this=_possibleConstructorReturn(this,(User.__proto__||Object.getPrototypeOf(User)).call(this));return render.createUser(index,split,oldPlayer),_this.betContainer="#betUser"+index,_this.cardContainer="#cardsUser"+index,_this.scoreContainer="#scoreUser"+index,_this.statusSplit=split,_this.userContainer="#user"+index,_this}return _inherits(User,_Player),_createClass(User,[{key:"checkDouble",value:function(){var check=0;this.aces>0&&(check=this.netScore-10),(this.netScore>=9&&this.netScore<=11||check>=9&&check<=11)&&buttons.toggleDouble()}},{key:"checkSplit",value:function(users,maxUsers){this.cards[0].value===this.cards[1].value&&users<maxUsers&&buttons.toggleSplit()}},{key:"determineOptions",value:function(users,maxUsers,balanceTotal){buttons.toggleStandAndHit(),2===this.cards.length&&this.bet<=balanceTotal&&(this.checkSplit(users,maxUsers),this.checkDouble())}},{key:"doubleBet",value:function(activePlayer){this.statusDouble=!0,render.bet(this.bet,activePlayer),this.bet=2*this.bet}},{key:"processDoubleScore",value:function(activePlayer){this.aces>0&&this.netScore>11&&(this.netScore-=10,render.score(activePlayer))}},{key:"removeHTML",value:function(obsoleteUser){render.removeUserHTML(obsoleteUser)}},{key:"removeSplitCard",value:function(player){var removedCard=this.cards[1];return this.grossScore=this.grossScore-removedCard.value,this.netScore=this.netScore-removedCard.value,"ace"===removedCard.number&&(this.aces--,this.netScore=this.netScore+10),this.cards.splice(1,1),render.removeSplitCard(player),removedCard}},{key:"setBet",value:function(bet,activePlayer){this.bet=bet,render.bet(bet,activePlayer)}}]),User}(Player);module.exports=User},function(module,exports,__webpack_require__){"use strict";var render=(__webpack_require__(1),__webpack_require__(0)),sounds=function(){function blackjack(){volumeOn&&(blackjackSound.pause(),blackjackSound.currentTime=0,blackjackSound.play())}function bust(){volumeOn&&(bustSound.pause(),bustSound.currentTime=0,bustSound.play())}function buttonPress(){volumeOn&&(buttonPressSound.pause(),buttonPressSound.currentTime=0,buttonPressSound.play())}function chips(){volumeOn&&(chipsSound.pause(),chipsSound.currentTime=0,chipsSound.play())}function draw(){volumeOn&&(drawSound.pause(),drawSound.currentTime=0,drawSound.play())}function drawCard(){volumeOn&&(drawCardSound.pause(),drawCardSound.currentTime=0,drawCardSound.play())}function gameOver(){volumeOn&&(gameOverSound.pause(),gameOverSound.currentTime=0,gameOverSound.play())}function lose(){volumeOn&&(loseSound.pause(),loseSound.currentTime=0,loseSound.play())}function toggleVolume(){volumeOn=!volumeOn,render.volumeMute(volumeOn)}function win(){volumeOn&&(winSound.pause(),winSound.currentTime=0,winSound.play())}var blackjackSound=document.createElement("audio"),bustSound=document.createElement("audio"),buttonPressSound=document.createElement("audio"),chipsSound=document.createElement("audio"),drawSound=document.createElement("audio"),drawCardSound=document.createElement("audio"),gameOverSound=document.createElement("audio"),loseSound=document.createElement("audio"),winSound=document.createElement("audio"),volumeOn=!0;return blackjackSound.setAttribute("src","./sounds/blackjack.wav"),bustSound.setAttribute("src","./sounds/bust.wav"),buttonPressSound.setAttribute("src","./sounds/buttonPress.mp3"),chipsSound.setAttribute("src","./sounds/chips.wav"),drawSound.setAttribute("src","./sounds/draw.wav"),
drawCardSound.setAttribute("src","./sounds/drawCard.wav"),gameOverSound.setAttribute("src","./sounds/gameOver.wav"),loseSound.setAttribute("src","./sounds/lose.wav"),winSound.setAttribute("src","./sounds/win.mp3"),{blackjack:blackjack,bust:bust,buttonPress:buttonPress,chips:chips,draw:draw,drawCard:drawCard,gameOver:gameOver,lose:lose,toggleVolume:toggleVolume,win:win}}();module.exports=sounds},function(module,exports,__webpack_require__){"use strict";var game=__webpack_require__(4),setOpeningBalance=1e3,setPacks=6,setUsersmin=1,setUsersmax=2;game.start(setOpeningBalance,setPacks,setUsersmin,setUsersmax)}]);